...
class PodcastInfo:
	...
	def episode_list(self):
		...
		for item in self.items_list:
			...
			#pub_date_d = item_pub_date[1]
			#pub_date_mn = item_pub_date[2]
			#pub_date_y = item_pub_date[3]
			#pub_date_h = item_pub_date[4].split(':')[0]
...
<rss version="2.0">
  <channel>
	...
	<item>
		...
		<enclosure url="{}" length="0" type="audio/mpeg"/>
		<guid>{}</guid>
	</item>\n""".format(episode.title, episode.pub_date_feed_txt(), episode.description, episode.url, episode.url)
	...
	def upload_image_to_server(self):
		...
		upload_cmd = '/home/pi/bin/upload_image.sh {}'.format(self.podcast.image)
		os.system(upload_cmd)
